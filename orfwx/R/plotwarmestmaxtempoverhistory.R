...
#' (or, if \code{daysWeather} is not provided, the year as of yesterday) is 
...
#' @examples
#' plotWarmestMaxTempOverHistory(airportData, searchDate(11, 26))  
#' # Returns plot for November 26th
#' @export
plotWarmestMaxTempOverHistory <- function(wxUniverse = orfwx::allData(),
                                          plotDate = yesterdate(),
                                          daysWeather = NULL,
                                          twoTicks = TRUE,
                                          fiveTicks = FALSE,
  ...
  if(saveToFile) {
    grDevices::png(paste0(format(plotDate, "%m%d"), "tmaxwrmst.png"),
