...
#' 
#' `plotCoolestMinTempOverHistory` creates a barplot with the coolest 
...
#' provided by `daysWeather`. (In all cases, the ten coolest years are 
#' shown, along with any ties, with additional years added until the year 
#' provided by `daysWeather` is shown, and also all years tied with the 
#' same temperature.) The bar for the year provided by `daysWeather` (or,
#' if `daysWeather` is not provided, the year as of yesterday) is 
...
plotCoolestMinTempOverHistory <- function(wxUniverse = orfwx::allData(),
  ...
  if(saveToFile) {
    grDevices::png(paste0(format(plotDate, "%m%d"), "tminclst.png"),
